...
ggplot(plot_filtered_dup_tnbc_data, aes(x = sample, y = expression, color = gene, group = gene)) +
  ...
  labs(title = "Y_RNA Gene Counts Across Samples",
       caption = "The distribution of raw counts across samples in genes annotated with HGNC symbol Y_RNA.",
       x = "Sample",
       y = "Counts") +
...
Roughly 14% of the data in each sample is an outlier.
...
There were 3 duplicate Ensembl IDs for the HGNC symbol `Y_RNA`, and after analyzing their trend across samples we decided to average their counts.
...
There were no replicate genes or samples.

**What is the final coverage of your dataset?**
In the process of cleaning and normalizing the data, we are left with `3690` genes in our bulk RNA-sequencing experiment.

In the process of cleaning and normalizing the data, we are left with `3690` genes in our bulk RNA-sequencing experiment. This dataset includes the neutrophils of 7 mTNBC patients in the test set and 7 healthy donors in the control group. All Ensembl IDs were mapped to HGNC symbols by the original authors. There were 3 duplicate Ensembl IDs for the HGNC symbol `Y_RNA`, and after analyzing their trend across samples we decided to average their counts. After normalization, there were 
